...
#include "MarkovChainApproximationX.h"
#include <cassert>
#include <algorithm>
#include <cmath>
#include <iostream>
...
MarkovChainApproximationX::MarkovChainApproximationX(MarkovChainParameters& mcp,
...
bool MarkovChainApproximationX::allStreamsOpen()
{
...
MarkovChainApproximationX::~MarkovChainApproximationX()
{
    if (memoryModeRAM_)
    {
        /*
...
void MarkovChainApproximationX::computeMarkovApproximation(const std::function<double(double*, double)>& costFunction,
...
void MarkovChainApproximationX::resetIndices(uint* currentIndices, uint padding)
{
    // We ignore boundaries
...
void MarkovChainApproximationX::solveTransitionSummations(std::vector<double>& v_summed,
                                                          uint* gridIndices,
...
double MarkovChainApproximationX::B_func(const std::function<double(double*, double)>& driftFunction,
                                         double* gridLocation,
                                         double alpha)
{
...
void MarkovChainApproximationX::solveTransitionProbabilities(std::vector<double>& v_probabilities,
                                                             uint* gridIndices,
...
double MarkovChainApproximationX::transitionProb(double* x,
...
double MarkovChainApproximationX::kahanSum(const vector<double>& doubleArray, size_t size_n)
{
...
void MarkovChainApproximationX::determineMinimumAlpha(const vector<double>& v_summed, uint* gridIndices)
{
...
bool MarkovChainApproximationX::nextRecursiveGrid(uint* currentIndices, uint padding)
{
...
uint MarkovChainApproximationX::recursionCount(uint dimIndex, 
                                               uint* indices,
                                               uint padding, 
                                               bool& stillInGrid)
{
    if (dimIndex >= 0)
    {
        ...
        else
        {
            ...
            dimIndex--;
            recursionCount(dimIndex, indices, padding, <#initializer#>);
        }
    }
    else 
    {
...
double MarkovChainApproximationX::getRelativeError()
{
    double temp;
    double maxValue = 0.0;
...
double MarkovChainApproximationX::getMarkovControlFunction(double* gridLocation)
{
    return (*minAlpha_)[getGridIndicesClosestTo(gridLocation)];
}

uint* MarkovChainApproximationX::getGridIndicesClosestTo(double* gridLocation)
{
    ...
    do
    {
        double* currentGridLoc = mcp_.getGridAtIndex(currentGridIndex);
